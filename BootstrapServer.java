...
public class BootstrapServer implements Serializable {
	...
	public BootstrapServer() {
		nsInfo = new NSInfo(0, serverPortForConnection);
		serverIDS.add(0);
	}
	...
	String lookup(int key) throws UnknownHostException, IOException, ClassNotFoundException {
		...
		if(key > Collections.max(serverIDS) && !data.containsKey(key)) {
			System.out.println("Server Visited: 0");
		...
		System.out.println("servers visited : ");
		...
		fwdSocket.close();
		return value.equalsIgnoreCase("NO")?"No key found":value;
	...
	void insert(int key, String value) throws IOException, ClassNotFoundException {
		...
		if (key > Collections.max(serverIDS)) {
			System.out.println("servers visited 0");
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		try {
			...
			UserFlow UI = new UserFlow(bootstrap);
			UI.start();
			int maxServerID = 0;
			while (true) {
				...
				if (!nameServerDetailsStr[0].equals("updateYourPredessorAndTakeAllKeys")) {
					if (!nameServerDetailsStr[0].equals("updateYourSuccessor")
							&& !nameServerDetailsStr[0].equals("updateMaxServerID")) {
				...
				case "entry":
					...
					if (bootstrap.nsInfo.getSuccessorId() == 0)// if only one server intial
					{
						...
						bootstrap.nsInfo.updateInformation(newNSListeningPort, newNSListeningPort, newNSId, newNSId,
								newNSIP, newNSIP);
					...
					} else if (maxServerID < newNSId) {// this means bootstrap server has keys for this ns i.e enter at
														// last
						...
						ObjectOutputStream outputStreamFwd = new ObjectOutputStream(fwdSocket.getOutputStream());
						outputStreamFwd
								.writeObject("entryAtLast " + newNSId + " " + newNSIP + " " + newNSListeningPort);
						...
						// 1)if new nameserver is between current server and successor and update the
						// successor of current server
						if (bootstrap.nsInfo.getSuccessorId() > newNSId)
							bootstrap.nsInfo.updateInformation(newNSListeningPort,
									bootstrap.nsInfo.predessorPortListning, newNSId, bootstrap.nsInfo.predessorId,
									newNSIP, bootstrap.nsInfo.predessorIP);

						// 2) if new server is not in between,or even if it is between contact the next
						// nameserver
						fwdSocket = new Socket(nextServerIP, nextServerListeningPort);
				...
				case "updateYourPredessorAndTakeAllKeys":
					...
					bootstrap.nsInfo.updateInformation(bootstrap.nsInfo.successorPortListning, predessorPortListning,
							bootstrap.nsInfo.getSuccessorId(), predessorId, bootstrap.nsInfo.successorIP, predessorIP);
					while (true) {
				...
				case "updateYourSuccessor":
					...
					bootstrap.nsInfo.updateInformation(successorPort, bootstrap.nsInfo.predessorPortListning,
							successorId, bootstrap.nsInfo.predessorId, successorIP, bootstrap.nsInfo.predessorIP);
					break;
...
//				System.out.println("BOOTSTRAP SuccessorId : " + bootstrap.nsInfo.successorId + " PredessorId :"
//						+ bootstrap.nsInfo.predessorId);
